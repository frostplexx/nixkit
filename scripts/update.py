...
def run_command(
    cmd: list[str], timeout: int = 600, capture: bool = True
) -> tuple[int, str]:
...
def update_package(package: str, create_pr: bool = False) -> UpdateResult:
    ...
    if create_pr:
        ...
        # Create new branch
        git_exit_code, _ = run_command(["git", "checkout", "-b", branch_name], capture=False)
        if git_exit_code != 0:
            return UpdateResult(
    ...
    cmd = ["nix-update", package, "--flake", "--build", "--commit",
    ...
    if update_exit_code != 0 and "Please specify the version" in output:
        ...
        if homepage_exit_code == 0 and homepage.strip():
            cmd = ["nix-update", package, "--flake", "--build", "--commit",
                   "--version=branch", "--url", homepage.strip()]
            update_exit_code, output = run_command(cmd, timeout=600)
        else:
...
def print_summary(results: list[UpdateResult]):
    ...
    print("\n" + "=" * 40)
    print(f"{Color.BLUE}📊 Update Summary{Color.NC}")
    print("=" * 40 + "\n")
...
def main():
    ...
    if not create_prs:
        ...
        git_status_exit_code, _ = run_command(["git", "diff-index", "--quiet", "HEAD", "--"])
